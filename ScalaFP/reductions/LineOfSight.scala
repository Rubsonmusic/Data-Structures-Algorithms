...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
	...
	var maxAngle = if (from==0) 0 else input(from)/from
	var i = from + 1
	while(i<until){
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if (end-from<=threshold)
	  ...
	  val (lr, rr) = parallel(upsweep(input, from, mid, threshold), upsweep(input, mid, end, threshold))
	  Node(lr, rr)
	}
  }
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
	...
	case node: Node => {
	  parallel(downsweep(input, output, startingAngle, node.left), downsweep(input, output, max(node.left.maxPrevious, startingAngle), node.right))
	  ()
    }
  }
