 ...
 */
Student::Student(const string& id, const int* history, int num, CP_TYPE cp_type):course_plan(NULL)
{
	this->id=id;
	for(int i=0; i<num; i++){
...
void Student::update_course_history(const map<int, Course>& course_db, int code)
{
	...
	map<int, Course>::const_iterator loc= course_db.find(code);

	if(loc!=course_db.end()){
...
void Student::print_course_history() const
{
    ...
    for(p=course_history.begin(); p!=course_history.end(); p++){
    	cout<<*p<<' ';
    }
    cout<<endl;
}
...
void Student::enroll(const map<int, Course>& course_db, int code)
{
	...
	for(int i=0; i<loc->second.get_num_prerequisites(); i++){
		if(course_history.find(loc->second.get_prerequisites(i))==course_history.end()){
			cout<<id<<": Can't enroll COMP"<<code<<". Not all pre-requisites are satisfied yet."<<endl;
			return;
		}
	}

	course_plan->insert(loc->second,code);
...
void Student::select_by_code(int base)
{
    ...
    while(!course_plan->iterator_end()){
    	const Course &temp=course_plan->iterator_next();
